...
title_agent = create_react_agent(
    ...
    Ensure that your excerpt is no more than 6 words. Make sure that the title is
    creative and properly summarizes the summary.
...
def title_node(state: State):
    ...
    for i in range(len(local_state.web_final_summaries)):
        ...
        local_state.messages = [(HumanMessage(content=f'The following is the summary of a web article: {local_state.web_final_summaries[i]}'))]
    ...
    for i in range(len(local_state.youtube_final_summaries)):
        print('Writing title for: ', local_state.desired_subjects[i])
        local_state.messages = [(HumanMessage(content=f'The following is the summary of a youtube video: {local_state.youtube_final_summaries[i]}'))]
        result = title_agent.invoke(local_state)
        new_message = result['messages'][-1].content
        local_state.youtube_final_titles.append(new_message)

    # Updating next in local state
    if (len(local_state.web_final_titles) > 0 and len(local_state.youtube_final_titles) > 0 and len(local_state.web_final_titles) == len(local_state.web_final_summaries) and len(local_state.youtube_final_titles) == len(local_state.youtube_final_summaries)):
