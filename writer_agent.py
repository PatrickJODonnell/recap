...
def writer_node(state: State):
    ...
    for i in range(len(local_state.web_summaries)):
        ...
        local_state.messages = [
            (HumanMessage(content=f"The following is the summary of a web article: {local_state.web_summaries[i]}"))
        ]
    ...
    for i in range(len(local_state.youtube_summaries)):
        ...
        local_state.messages = [
            (
                HumanMessage(
                    content=f"The following is the summary of a youtube video: {local_state.youtube_summaries[i]}"
                )
            )
        ]
        result = writer_agent.invoke(local_state)
