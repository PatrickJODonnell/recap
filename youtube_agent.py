...
from utils import cosine_similarity, get_openai_embedding, retry, scrape_with_webbase


# Getting youtube video audio (native format, no ffmpeg)
def get_youtube_audio(video_url: str) -> str:
    # Downloading the audio of the video in native format (m4a/webm) without conversion
    audio_file_name = f"audio_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    audio_ydl_opts = {
        "format": "bestaudio[ext=m4a]/bestaudio[ext=webm]/bestaudio/best",
        "outtmpl": f"{audio_file_name}.%(ext)s",
    ...
    # Discover the actual file path (extension is set by yt-dlp)
    matches = glob.glob(f"{audio_file_name}.*")
    if len(matches) != 1:
        raise RuntimeError(
            f"Expected exactly one downloaded audio file matching {audio_file_name}.*, got {len(matches)}: {matches}"
        )
    return matches[0]
...
def video_search(query: str, api_key, count=10, country="US", lang="en") -> List[dict]:
    ...
    if response.status_code == 200:
        ...
        for video in result["results"]:
            ...
            # Checking relevency
            if (
                (stripped_query in selected_title or stripped_query in selected_description)
                and ("https://www.youtube.com" in selected_video)
                and (selected_length is not None and isTimeValid)
                and (isRecent)
            ):
                ...
                (title_similarity > 0.78 or desc_similarity > 0.78)
                and ("https://www.youtube.com" in selected_video)
                and (selected_length is not None and isTimeValid)
                and (isRecent)
            ):
...
def youtube_node(state: State):
    ...
    for i in range(len(local_state.youtube_queries)):
        ...
        if scrape_result.strip() != "404 Not Found" and scrape_result is not None:
            # This is a youtube profile. Grabbing the most recent video (if it exists)
            ...
            try:
                ...
                print(f"Error pulling video from youtube profile, attempting generic video search: {e}")

            # If the video is found, transcribe and summarize it
            if chosen_video is not None:
                print("Video found.")
                # Pulling the chosen video's audio
                audio_path = get_youtube_audio(chosen_video["url"])
                ...
                local_state.messages = [
                    (
                        HumanMessage(
                            content=f"The video title: {chosen_video['title']} | The video transcript {transcript}"
                        )
                    )
                ]
        ...
        if video_urls is None:
            ...
            time.sleep(1)
            video_urls = video_search(
                local_state.web_queries[i] + " news", api_key=os.getenv("BRAVE_API_KEY"), count=20
            )
            if video_urls is None:
        ...
        if chosen_video:
            ...
            local_state.messages = [
                (HumanMessage(content=f"The video title: {chosen_video['title']} | The video transcript {transcript}"))
            ]
            result = youtube_agent.invoke(local_state)
            new_message = result["messages"][-1].content
    ...
    if (
        len(local_state.youtube_links) > 0
        and len(local_state.youtube_summaries) > 0
        and len(local_state.youtube_links) == len(local_state.youtube_summaries)
    ):
